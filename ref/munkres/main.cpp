...
void test_generator(std::vector< std::vector<int> > &x)
{
	...
	const int num_rows = ((rand() % (max_size-(min_size-1))) + min_size);
	const int num_columns = ((rand() % (max_size-(min_size-1))) + min_size);
...
int main2(int argc, char * const argv[]) {
    ...
    std::vector<ordered_pair> p;
    int cost = test.assign(p);
	...
	// std::cerr << "Dimensions: (" << p.size() << ", " << num_columns << ")" << std::endl;
